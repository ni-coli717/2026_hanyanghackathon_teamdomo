...
MODEL_PATH = DATA_DIR / "models" / "latest.joblib"
...
def render_analysis(reports, windows, sources):
    ...
    st.markdown(f'<div class="card-grid">{card("관측 기간", f"{reports.observed_at.min()[:10]} ~ {reports.observed_at.max()[:10]}")}{card("전체 기록", f"{len(reports):,}건", f"유효 관측창 {len(valid):,}개")}{card("집단 감지 창", f"{int(valid.label.sum()):,}개", f"미정 {int(windows.label.isna().sum()):,}개")}</div>', unsafe_allow_html=True)
...
def render_device(repo, probability, level):
    ...
    with settings_col:
        ...
        st.caption("시작 0.65 · 해제 0.45 · 히스테리시스 적용")
    ...
    command, next_state, reason = device_command(p, st.session_state.fan_on, manual_stop, is_quiet, duration_reached)
    with state_col:
        st.markdown("#### 기기 상태")
        ...
        if st.button("현재 위험도로 명령 평가", use_container_width=True, type="primary"):
            ...
            send_command = command or f"RISK:{level if level != 'HOLD' else 'LOW'}"
...
pages = ["① 홈", "② 동네 지도", "③ 냄새 기록", "④ 분석·방향", "⑤ AI 실험", "⑥ 우리 집 대응", "⚙ 설정"]
